...
namespace EInfrastructure.Core.Words.Extension.ImportDict.Common.Entities
{
    ...
    internal class WordLibrary
    {
        ...
        public string[] PinYin
        {
            get
            {
                if ((CodeType == CodeTypeEnum.Pinyin || CodeType == CodeTypeEnum.Zhuyin || CodeType == CodeTypeEnum.TerraPinyin) &&
                    Codes.Count > 0)
                {
        ...
        public string WubiCode
        {
            get
            {
                if (CodeType == CodeTypeEnum.Wubi||CodeType==CodeTypeEnum.Wubi98)
                {
                    return Codes[0][0];
                }
                return null;
