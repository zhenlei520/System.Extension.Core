        ...
        public UserAgentBase Execute()
        {
                ...
                uaData.Device.DeviceType.Equals(DeviceType.Tablet))
            {
                    ...
                    ).Length > 0)
                {
                    ...
                    if ((mc = UA.Match(@"(\w*)[\s-_]+[a-z0-9]+",RegexOptions.IgnoreCase)).Length > 0)
                    {
                        uaData.Device.Name = mc[1].SafeString();
                    }
                }
                ...
                else if ((mc = UA.Match(@";\s(mi|m1|m2|m3|m4|hm)(\s*\w*)[\s\)]",RegexOptions.IgnoreCase)).Length > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!string.IsNullOrEmpty(mc[2].SafeString()) && mc[2].SafeString().Length > 0)
                        {
                            ...
                            uaData.Device.Name =
                                (mc[1].Substring(mc[1].Length - 2) + '-' + mc[2]).ReplaceRegex(@"m(\d)-",RegexOptions.IgnoreCase, "MI-$1");
                        }
                        else
                        {
                            uaData.Device.Name = (mc[1].Substring(mc[1].Length - 2)).ReplaceRegex(@"m(\d)",RegexOptions.IgnoreCase, "MI-$1");
                        }
                        ...
                        {
                            // 看看是不是 MI-3S  MI-4S....
                            if ((mc = uaData.Device.Name.Match(@"(mi|hm)(-\ds)",RegexOptions.IgnoreCase)).Length > 0)
                            {
                                uaData.Device.Name = mc[1] + mc[2];
                            }
                            // 防止 MI-20150XX等滥竽充数成为MI-2
                            else if ((mc = uaData.Device.Name.Match(@"(mi|hm)(-\d{2})",RegexOptions.IgnoreCase)).Length > 0)
                            {
                                uaData.Device.Name = mc[1];
                            }
                            // 将mi-3c mi-3a mi-3w等合为mi-3
                            else if ((mc = uaData.Device.Name.Match(@"(mi|hm)(-\d)[A-Z]",RegexOptions.IgnoreCase)).Length > 0)
                            {
                                uaData.Device.Name = mc[1] + mc[2];
                            }
                        }
                        ...
                        // 去除 mi-4g这样的东西
                        if ((mc = uaData.Device.Name.Match(@"(mi|hm)(-\dg)",RegexOptions.IgnoreCase)).Length > 0)
                        {
                            uaData.Device.Name = mc[1];
                        }
                    }
                ...
                else if ((mc = UA.Match(@"redmi\s?(\d+)?",RegexOptions.IgnoreCase)).Length > 0)
                {
                    uaData.Device.Manufacturer = "Xiaomi";
                    ...
                    "xiaomi" && !uaData.Device.Name.IsNullOrEmpty())
                {
                    ...
                    if ((mc = uaData.Device.Name.Match(@"mi-one",RegexOptions.IgnoreCase)).Length > 0)
                    {
                        uaData.Device.Name = "MI-1";
                    }
                    // mi 2
                    else if ((mc = uaData.Device.Name.Match(@"mi-two",RegexOptions.IgnoreCase)).Length > 0)
                    {
                        uaData.Device.Name = "MI-2";
                    }
                    // 20150xxx2014501
                    else if ((mc = uaData.Device.Name.Match(@"\d{6}",RegexOptions.IgnoreCase)).Length > 0)
                    {
                        uaData.Device.Name = "";
                    }
                    else if ((mc = uaData.Device.Name.Match(@"redmi",RegexOptions.IgnoreCase)).Length > 0)
                    {
                        uaData.Device.Name = uaData.Device.Name.ToUpper().ReplaceRegex(@"redmi",RegexOptions.IgnoreCase, "HM");
                    }
                    // m1 m2 m3 写法不标准 另外判断是否是 m1-s
                    else if ((mc = uaData.Device.Name.Match(@"(m\d)[\s-_](s?)",RegexOptions.IgnoreCase)).Length > 0)
                    {
                        uaData.Device.Name = mc[1].ReplaceRegex(@"m", "MI-") + mc[2];
                    }
                    // mi-2w  mi-3w 等格式化为mi-2  mi-3
                    else if ((mc = uaData.Device.Name.Match(@"(hm|mi)[\s-_](\d?)[a-rt-z]",RegexOptions.IgnoreCase)).Length > 0)
                    {
                        if ((tempMc = uaData.Device.Name.Match(@"(hm|mi)[\s-_](\d?)[a-rt-z]",RegexOptions.IgnoreCase)).Length > 0)
                        {
                    ...
                    // 处理hm
                    else if ((mc = uaData.Device.Name.Match(@"hm",RegexOptions.IgnoreCase)).Length > 0)
                    {
                        // 判断是不是 hm-201xxx充数
                        if ((mc = uaData.Device.Name.Match(@"(hm)[\s-_](\d{2})",RegexOptions.IgnoreCase)).Length > 0)
                        {
                            uaData.Device.Name = "HM";
                        }
                        // 判断是不是 hm-2s hm-1s
                        else if ((mc = uaData.Device.Name.Match(@"(hm)[\s-_](\ds)",RegexOptions.IgnoreCase)).Length > 0)
                        {
                            uaData.Device.Name = "HM-" + mc[2];
                        }
                        else if ((mc = uaData.Device.Name.Match(@"(hm)[\s-_](\d)[a-z]",RegexOptions.IgnoreCase)).Length > 0)
                        {
                            uaData.Device.Name = "HM-" + mc[2];
                        }
                        else
                    ...
                    ).Length > 0)
                {
                    ...
                    // 对 Plus 做特殊处理
                    if ((mc = uaData.Device.Name.Match(@"([a-z]+[0-9]+)-?(plus)",RegexOptions.IgnoreCase)).Length > 0)
                    {
                        uaData.Device.Name = mc[1] + '-' + mc[2];
                    }
                ...
                // handle meizu
                else if ((mc = UA.Match(@"\s(mx\d*\w*|mz-(\w*))\s(\w*)\s*\w*\s*build",RegexOptions.IgnoreCase)).Length > 0)
                {
                ...
                else if ((mc = UA.Match(@"M463C|M35\d",RegexOptions.IgnoreCase)).Length > 0)
                {
                    uaData.Device.Manufacturer = "Meizu";
                    uaData.Device.Name = mc[1];
                }
                ...
                // UIMI
                else if ((mc = UA.Match(@"(UIMI\w*|umi\w*)[\s-_](\w*)\s*\w*\s*build",RegexOptions.IgnoreCase)).Length > 0)
                {
                ...
                // eton
                else if ((mc = UA.Match(@"eton[\s-_](\w*)",RegexOptions.IgnoreCase)).Length > 0)
                {
...
// handle nubia
                ...
                // handle haier
                else if ((mc = UA.Match(@"(HT-\w*)|Haier[\s-_](\w*-?\w*)",RegexOptions.IgnoreCase)).Length > 0)
                {
                ...
                // tianyu
                else if ((mc = UA.Match(@"K-Touch[\s-_](tou\s?ch\s?(\d)|\w*)",RegexOptions.IgnoreCase)).Length > 0)
                {
                ...
                // DOOV
                else if ((mc = UA.Match(@"Doov[\s-_](\w*)",RegexOptions.IgnoreCase)).Length > 0)
                {
                ...
                // haisense
                else if ((mc = UA.Match(@"(hs-|Hisense[\s-_])(\w*)",RegexOptions.IgnoreCase)).Length > 0)
                {
                ...
                {
                    if ((mc = uaData.Device.Name.Match(@"(hm|mi)-(note)",RegexOptions.IgnoreCase)).Length > 0)
                    {
                        uaData.Device.Name = mc[1] + '-' + mc[2];
                    }
                    else if ((mc = uaData.Device.Name.Match(@"(hm|mi)-(\ds?)",RegexOptions.IgnoreCase)).Length > 0)
                    {
                        uaData.Device.Name = mc[1] + '-' + mc[2];
                    }
                    else if ((mc = uaData.Device.Name.Match(@"(hm|mi)-(\d)[a-rt-z]",RegexOptions.IgnoreCase)).Length > 0)
                    {
                        uaData.Device.Name = mc[1] + '-' + mc[2];
                    }
                }
            ...
            if (uaData.Device.DeviceType.Equals(DeviceType.Pc))
            {
                 ...
                 * LBBROWSER
                 */
                else if ((mc = UA.Match(@"LBBROWSER",RegexOptions.IgnoreCase)).Length > 0)
                {
                     ...
                     uaData.Device.DeviceType.Equals(DeviceType.Tablet))
            {
                 ...
                 */
                         ...
                         ((mc = UA.Match(@"chrome\/([0-9\.]+)",RegexOptions.IgnoreCase)).Length > 0))
                {
                 ...
                 */
                         ...
                         ((mc = UA.Match(@"version\/([0-9\.]+)",RegexOptions.IgnoreCase)).Length > 0))
                {
                    if ((tempMc = UA.Match(@"\)s+(\w+Browser)\/?([\d\.]*)")).Length > 0)
                    {
                        uaData.Browser.Name = tempMc[1];
            ...
            if ((mc = UA.Match(@"baiduboxapp\/?([\d\.]*)",RegexOptions.IgnoreCase)).Length > 0)
            {
            ...
            // }
            if ((mc = UA.Match(@"MicroMessenger\/([\w.]+)",RegexOptions.IgnoreCase)).Length > 0)
            {
            ...
            {
                if ((mc = UA.Match(@"Safari\/([\w.]+)",RegexOptions.IgnoreCase)).Length > 0 && UA.Test(@"Version",RegexOptions.IgnoreCase))
                {
                    uaData.Browser.Name = "Safari";
                }
            }
            ...
            else if (uaData.Os.Name == "Mac OS X")
            {
                ...
                if ((mc = UA.Match(@"Mac OS X[\s\_\-\/](\d+[\.\-\_]\d+[\.\-\_]?\d*)",RegexOptions.IgnoreCase)).Length > 0)
                {
                    uaData.Os.Alias = mc[1].ReplaceRegex(@"_",RegexOptions.Compiled, ".");
                    uaData.Os.Version = new Versions()
                    {
                        Original = mc[1].ReplaceRegex(@"_",RegexOptions.Compiled, ".")
                    };
                }
                else
                {
            ...
            {
                if ((mc = UA.Match(@"Android[\s\\_\-\/i686]?[\s\\_\-\/](\d+[\.\-\\_]\d+[\.\-\\_]?\d*)",RegexOptions.IgnoreCase)).Length > 0)
                {
                    uaData.Os.Alias = mc[0].Split('/')[0];
                    uaData.Os.Version = new Versions(mc[0].Match(uaData._regexConfigurations.GetRegexRule(RegexDefault.FloatingPointNumbers))[0]);
                }
            }
