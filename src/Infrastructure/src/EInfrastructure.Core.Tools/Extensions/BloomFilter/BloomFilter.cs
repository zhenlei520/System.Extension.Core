...
namespace EInfrastructure.Core.Tools.Extensions.BloomFilter
{
    ...
    {
        private BitArray _bitset;
        private int _bitSetSize;
        private double _bitsPerElement;
        private int _expectedNumberOfFilterElements; // expected (maximum) number of elements to be added
        ...
        {
            this._expectedNumberOfFilterElements = n;
            this._k = k;
            this._bitsPerElement = c;
            this._bitSetSize = (int) Math.Ceiling(c * n);
            _numberOfAddedElements = 0;
            this._bitset = new BitArray(_bitSetSize);
        }
        ...
        {
            this._bitset = filterData;
            this._numberOfAddedElements = actualNumberOfFilterElements;
        }
        ...
        public override bool Equals(object obj)
        {
            ...
            if (this._expectedNumberOfFilterElements != other._expectedNumberOfFilterElements)
            {
                return false;
            }
            ...
            if (this._bitSetSize != other._bitSetSize)
            {
                return false;
            }

            if (this._bitset != other._bitset &&
                (this._bitset == null || !ObjectUtils.Equals(this._bitset, other._bitset)))
            {
                return false;
            }
        ...
        {
            int hash = 7;
            hash = 61 * hash + (this._bitset != null ? ObjectUtils.HashBytes(this._bitset) : 0);
            hash = 61 * hash + this._expectedNumberOfFilterElements;
            hash = 61 * hash + this._bitSetSize;
            hash = 61 * hash + this._k;
            return hash;
        }
        ...
        {
            // (1 - e^(-k * n / m)) ^ k
            return Math.Pow((1 - Math.Exp(-_k * (double) numberOfElements
                                          / (double) _bitSetSize)), _k);
        }
        ...
        public void Add(byte[] bytes)
        {
            ...
            {
                _bitset.Set(MathUtils.Abs(hash % _bitSetSize), true);
            }
        ...
        public bool Contains(byte[] bytes)
        {
            ...
            foreach (int hash in hashes)
            {
                if (!_bitset.Get(MathUtils.Abs(hash % _bitSetSize)))
                {
                    return false;
                }
            }
        ...
        public int Count()
        {
            return this._numberOfAddedElements;
        }
        ...
        {
            return this._bitSetSize / (double) _numberOfAddedElements;
        }
    }
}
